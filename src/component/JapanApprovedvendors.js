...
function JapanApprovedvendors() {
    ...
    useEffect(() => {
        getJapanApprovedStat();
    }, [])

    const getJapanApprovedStat = async () => {
        const Level2approved = [];
        apiService.getApprovalList().then((res) => {
            setFilter(res.data.result);
            if (res.data.result) {
                res.data.result.forEach((item) => {
                    if (item.level2Status === "approved") {
                        var date1 = new Date();
                        ...
                        item.level2Date = s;
                        Level2approved.push(item);
                    }
                });
                setvendors([]);
                setvendors((array) => [...array, ...Level2approved]);
            }
        });
    };
